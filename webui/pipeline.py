...
def _stream(cmd, cwd: Path, env: dict):
    ...
    proc = subprocess.Popen(
        ...
        text=True,
        bufsize=1,
        # Force UTF-8 with replacement so non-cp1252 output (smart quotes,
        # em-dashes, accented/non-Latin text) cannot crash the reader on Windows.
        encoding="utf-8",
        errors="replace",
    )
...
def _utf8_env() -> dict:
    """A copy of the environment that forces the child Python to use UTF-8.
 
    On Windows a subprocess's stdout defaults to cp1252, so ``print``-ing text
    that contains characters like ``π`` (\\u03c0) raises UnicodeEncodeError and
    aborts the whole script before it can write storyboard.json. Forcing UTF-8
    makes the child scripts robust to any document content.
    """
    env = os.environ.copy()
    env["PYTHONUTF8"] = "1"
