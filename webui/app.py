...
def _kaggle_status_md() -> str:
    ...
    return (
        ...
        "No Kaggle credentials — add ~/.kaggle/access_token (new token system) or "
        "set KAGGLE_USERNAME + KAGGLE_KEY / add ~/.kaggle/kaggle.json to enable the "
        "video stage. Storyboard + images still work."
        "</div>"
    )
...
if __name__ == "__main__":
    ...
    build_ui().queue().launch(
        server_name="0.0.0.0", server_port=server_port, theme=THEME, css=CSS, share=True
    )
